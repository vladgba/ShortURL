...
if (isset($_GET['url']) && $_GET['url'] != "") {
    ...
    echo '<input type="text" value="' . $base_url . '/' . $slug . '" id="myInput"><button onclick="myFunction()" onmouseout="outFunc()">Copy URL</button></center>';
}
else {
...
function GetRedirectUrl($conn, $slug) {
    try {
        ...
        $quer->execute(array(addslashes($slug)));
        if ($quer->rowCount() > 0) {
            $res = $quer->fetch(PDO::FETCH_ASSOC);
            return $res['url'];
        }
